...
class ValidEvent:
    ...
    def check_json_is_list(self, json_data):
        # проверка входного сериализованного типа
    ...
    def check_key_values(self, event):
        # проверка наличия ключей и содержимого
    ...
    def is_iso_datetime(self, date):
        try:
            datetime.fromisoformat(date)
        except Exception:
            raise ValueError("Date format does not match ISO: {}".format(date))

    def is_event_type(self, type):
        try:
            EventType[type.upper()]
        except Exception:
            raise ValueError("Unsupported event format: {}".format(type))

    def is_event_name(self, name):
        if not isinstance(name, str):
            raise ValueError("Event name should be string")
